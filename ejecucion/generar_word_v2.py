...
Corrección: Ajusta numeración incorrecta en algunas fichas
"""
...
def fix_section_number(text, ficha_num):
    """Corrige la numeración de secciones incorrectas"""
    # Si es una sección como 8.1.X, 8.2.X, etc. pero debería ser 8.14.X, 8.15.X
    # basado en el número de ficha
    if ficha_num >= 14:
        # Patrón para detectar secciones mal numeradas
        pattern = r'^8\.(\d+)\.'
        match = re.match(pattern, text)
        if match:
            subnum = match.group(1)
            # Si la subsección es 1-9 pero la ficha es >=14, corregir
            if subnum in ['1', '2', '3', '4', '5', '6', '7', '8', '9']:
                # Reemplazar 8.X. con 8.{ficha_num}.
                text = re.sub(r'^8\.\d+\.', f'8.{ficha_num}.', text)
    return text
...
def parse_table_lines(lines, start_idx):
    ...
    while i < len(lines):
        ...
        if line.replace('|', '').replace('-', '').replace(' ', '') == '':
            i += 1
            continue
        # Saltar líneas que solo tienen guiones
...
def process_ficha(doc, filepath, ficha_num):
    ...
    # Corregir numeración en el contenido antes de procesar
    lines = content.split('\n')
    i = 0
    in_table = False
    table_rows = []
    ...
    while i < len(lines):
        ...
        if stripped.startswith('# ') and not stripped.startswith('# 8.'):
            ...
            if text:
                # Corregir numeración si es necesario
                text = fix_section_number(text, ficha_num)
