...
# Template base para sistemas sin contenido específico
def generar_contenido_base(codigo, nombre, desc):
    return {
        "alcance": f"Diseño, suministro, instalación, configuración y puesta en marcha del sistema {nombre} ({desc}) para las 306 km del corredor vial Chía-Girardot, incluyendo todos los componentes necesarios para su operación integral dentro del ecosistema ITS.",
        "descripcion": f"Sistema {nombre} diseñado específicamente para las condiciones operativas del corredor Chía-Girardot, considerando sus 306 km de extensión, zonas de neblina persistente, geotecnia compleja con zonas inestables, clima diverso desde altiplano con heladas hasta valles cálidos, y alta siniestralidad histórica. El sistema integra tecnologías robustas para operación continua 24/7/365 en condiciones ambientales adversas.",
        "justificacion": f"El sistema {nombre} es fundamental para la operación segura y eficiente del corredor Chía-Girardot. Su implementación contribuye directamente a la reducción de la siniestralidad, mejora en los tiempos de respuesta ante incidentes, optimización de la gestión del tráfico, y cumplimiento de normativas del Ministerio de Transporte. La integración con el modelo BIM de la infraestructura permite mantenimiento predictivo y gestión integral del corredor.",
        "em": [f"EM-01: Especificación mínima de rendimiento para {nombre} garantizando disponibilidad superior al 99%","EM-02: Cumplimiento con normas técnicas NTC e ISO aplicables a sistemas ITS","EM-03: Compatibilidad con protocolo NTCIP para interoperabilidad de sistemas","EM-04: Operación en rango de temperatura entre -5°C y 45°C con humedad hasta 95%","EM-05: Protección IP65/IP66 contra polvo, lluvia y condiciones climáticas adversas","EM-06: Resistencia a vibraciones y condiciones de carretera de montaña","EM-07: Comunicación redundante con respaldo automático ante fallas","EM-08: Integración nativa con el Centro de Control Operativo (CCO)","EM-09: Monitoreo remoto del estado operativo y diagnóstico de fallas","EM-10: Documentación técnica completa y manuales de operación"],
        "rf": [f"RF-01: Interfaz de usuario intuitiva para operadores del CCO con mapa georeferenciado","RF-02: Generación automática de alertas ante condiciones anómalas o fallas del sistema","RF-03: Capacidad de configuración remota de parámetros operativos","RF-04: Integración bidireccional con otros sistemas ITS del corredor","RF-05: Exportación de datos en formatos estándar para análisis e informes","RF-06: Gestión de usuarios con roles diferenciados y auditoría de acciones","RF-07: Dashboard en tiempo real del estado operativo del sistema","RF-08: Reportes programables de desempeño, disponibilidad y eventos","RF-09: API REST para integración con sistemas de terceros","RF-10: Soporte para visualización móvil para personal autorizado"],
        "rnf": [f"RNF-01: Tiempo de respuesta de la interfaz inferior a 2 segundos para cualquier operación","RNF-02: Capacidad para soportar mínimo 100 usuarios concurrentes sin degradación","RNF-03: Cifrado de datos en tránsito y en reposo según estándares de la industria","RNF-04: Arquitectura escalable que permita expansión futura sin rediseño","RNF-05: Tiempo medio entre fallos (MTBF) superior a 30,000 horas","RNF-06: Tiempo medio de reparación (MTTR) inferior a 4 horas","RNF-07: Cumplimiento con ISO 27001 para seguridad de la información","RNF-08: Disponibilidad del sistema superior al 99.5% anual","RNF-09: Tolerancia a fallos con conmutación automática a componentes de respaldo","RNF-10: Soporte técnico 24/7/365 con SLA garantizado"],
        "kpi": [f"KPI-01: Disponibilidad operativa del sistema {nombre}, meta superior al 99.5%","KPI-02: Tiempo promedio de respuesta ante eventos, meta inferior a 30 segundos","KPI-03: Tiempo medio entre fallos (MTBF), meta superior a 30,000 horas","KPI-04: Tiempo medio de reparación (MTTR), meta inferior a 4 horas","KPI-05: Índice de satisfacción de usuarios operadores, evaluación trimestral","KPI-06: Cumplimiento de acuerdos de nivel de servicio (SLA)","KPI-07: Número de incidentes de seguridad detectados y mitigados","KPI-08: Costo operativo por kilómetro de corredor atendido","KPI-09: Eficiencia energética del sistema en operación continua","KPI-10: Reducción de siniestralidad atribuible al sistema implementado"]
    }

# Contenido específico por sistema
...
print("Generando Fichas ITS Completas - Corredor Chía-Girardot")
print("=" * 55)
...
for codigo, nombre, desc in SISTEMAS:
    ...
    if codigo in CONTENIDO_ESPECIFICO:
        data = CONTENIDO_ESPECIFICO[codigo]
    else:
        data = generar_contenido_base(codigo, nombre, desc)
    ...
    ws['B6'] = data["descripcion"]
    ws['A7'] = "Justificación Estratégica"
    ws['B7'] = data["justificacion"]
    
    for i in range(10):
        ws[f'A{8+i}'] = f"EM-{i+1:02d}"
        ws[f'B{8+i}'] = data["em"][i]
    for i in range(10):
        ws[f'A{18+i}'] = f"RF-{i+1:02d}"
        ws[f'B{18+i}'] = data["rf"][i]
    for i in range(10):
        ws[f'A{30+i}'] = f"RNF-{i+1:02d}"
        ws[f'B{30+i}'] = data["rnf"][i]
    for i in range(10):
        ws[f'A{42+i}'] = f"KPI-{i+1:02d}"
        ws[f'B{42+i}'] = data["kpi"][i]
...
print(f"✓ Total de hojas generadas: {len(SISTEMAS)}")
